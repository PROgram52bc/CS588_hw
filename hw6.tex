...
\begin{enumerate}
    ...
    \begin{proof}
        ...
        theorem 21.5: Let $S = (\mathds{1} \otimes \mathds{1})/\vert F\vert^2$, then $R = S + E$,
        where $E = R - S$ is the non-stationary portion of $R$ in the form of
        ...
        where each $R_i$ contains entries either $-1$ or $\frac{1}{\vert F\vert}
        - 1$. More specifically, each $R_i - 1$ is of the form 
        \[ R_i - S_i = R_i - \frac{1}{\vert F\vert^2} = \begin{bmatrix}
        ...
        Note that $S_i$ is the stationary component of $R_i - S_i$
        
        compute eigenvalues, conclude that it is at most $\frac{1}{\vert
        F\vert}$. Thus the second largest eigenvalue of $G^2$ is at most
        $\lambda(G^2) = \frac{1}{\vert F\vert}$.
...
\begin{enumerate}
    \item \begin{proof}
        ...
        $$\mathds{1}_S^T A \mathds{1}_S = 0$$

        TODO: split $\mathds{1}_S$ into two components, parallel and orthogonal
        to the 1 vector (Why?)
        \end{proof}
    \item \begin{proof}
    ...
    \item \begin{proof}
        ...
        $R^ks_0$ has no zero entries, which means every vertex has a non-zero
        probability of being explored after $k$ steps.

        We solve for $(R^ks_0)_v > 0$. We
